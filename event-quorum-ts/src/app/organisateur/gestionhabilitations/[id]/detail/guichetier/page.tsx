// File: src/app/organisateur/gestionhabilitations/[id]/detail/guichetier/page.tsx
'use client'
...
/**
 * Page de détail pour un utilisateur avec le rôle Guichetier
 * Affiche les informations personnelles et les permissions spécifiques au rôle
 */
const GuichetierDetailPage: React.FC = () => {
    ...
    const [userData, setUserData] = useState<GuichetierData | null>(null);

    // Données mockées spécifiques au Guichetier pour le développement
    const mockGuichetierData: GuichetierData = {
        id: 4,
        nom: 'Traore',
        prenom: 'Ibrahim',
        ...
        permissions: {
            ...
            ecriture: true,
            modification: false,
            // Permission spécifique Guichetier
            ajouterParticipants: true,
        }
    };

    /**
     * Charge les données du guichetier depuis l'API
     * Actuellement utilise des données mockées pour le développement
     */
    useEffect(() => {
        const loadGuichetierData = async () => {
            try {
                setLoading(true);

                // TODO: Remplacer par l'appel API réel
                // const response = await fetch(`/api/authorizations/guichetier/${authId}`);
                // const data = await response.json();

                // Simulation d'un délai réseau
                await new Promise(resolve => setTimeout(resolve, 800));
                setUserData(mockGuichetierData);

            } catch (error) {
                console.error('Erreur lors du chargement des données guichetier:', error);
            } finally {
                setLoading(false);
            }
        };
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        <Box>
                            ...
                            <Typography variant="body2" color="text.secondary">
                                Informations et permissions du guichetier
                            </Typography>
                        </Box>
                    </Box>
                    ...
                    <Grid item xs={12} md={8}>
                        <Card>
                            ...
                            <Box sx={{ p: 3 }}>
                                ...
                                {/* Bloc Participants - Spécifique Guichetier */}
                                <Box sx={{ mb: 3 }}>
                                    <Typography variant="subtitle1" sx={{
                                        fontWeight: 600,
                                        mb: 2,
                                        ...
                                        <Table size="small">
                                            <TableBody>
                                                <TableRow>
                                                    ...
                                                    <TableCell sx={{ border: 'none', py: 1.5, width: 120 }} align="right">
                                                        {renderPermissionStatus(userData.permissions.ajouterParticipants)}
                                                    </TableCell>
                                                </TableRow>
                                            </TableBody>
                                        </Table>
...
export default GuichetierDetailPage;